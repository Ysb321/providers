...
| XDMovies | `xdMovies` | https://top.xdmovies.wtf |
...
`providers/xdMovies/` targets a listing site whose detail pages carry direct
release files (TMDB-backed metadata, `S01E02`-style filenames).
...
- `meta.ts` – title, poster, rating, genres, cast and synopsis. Release
  filenames are parsed for `SxxExx` and resolution, so a series is grouped into
  `Season N` buckets (episodes ordered, quality-sorted) with season packs kept
  separately. Movies list one entry per release, best quality first.
- `stream.ts` – dispatches by host (see below).
- `settings.ts` – site mirror override (`xdMoviesBaseUrl`).

### Two kinds of download link

Detail pages mix two host families, and they behave very differently:

| Host | Handling |
| --- | --- |
| `hubcloud.one` (and siblings) | Resolved directly by the vendored `extractors/hubcloud.ts`, with mirror fallback on connection failures |
| `gdflix`, `gofile`, `filepress`/`filebee` | Routed to their matching extractor |
| `link.xdmovies.wtf` | The site's own shortener - fronted by **Cloudflare Turnstile plus a countdown**, so plain HTTP cannot resolve it |

Because the shortener needs a real browser, `stream.ts` **prefers non-gated
hosts**: when a title offers both, HubCloud is tried first and the gate is never
touched. If only gated links exist, the app WebView runs the verification and
whatever file-host URL appears afterwards is handed to the matching extractor.

If that fails without a WebView available (for example under the CLI test
runner), the error says so explicitly rather than blaming the network.
