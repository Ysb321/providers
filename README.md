...
## `example`
The template's built-in sample provider (mock data). Keep it as a reference, or delete it and model your own providers on its structure.

## `roshy`
A **movies / TV series provider template** following the template conventions
(`catalog.ts`, `posts.ts`, `meta.ts`, `stream.ts`, `settings.ts`). It scrapes
standard WordPress movie/TV theme markup (`.movie-card`, `.film-item`,
`article.post-item`, `h1.page-title`) with pagination and `?s=` search.

> **Configuration:** Its base URL is intentionally left **blank**. Set the
> domain of the **non‑adult** movies/TV site you want to scrape either directly
> in `providers/roshy/posts.ts`/`meta.ts` (`defaultBaseUrl`) or through the
> `baseUrlOverride` setting in the Vega Provider Manager, and adjust the CSS
> selectors if the target site differs.
>
> The `roshy.tv` domain currently serves adult (JAV) content and is **not**
> wired here; this provider is not intended for that content.
